        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                SettingsGroup("Widget Look") {
                    Picker("Vinyl style", selection: $settings.vinylStyle) {
...
final class SettingsWindowController {
    ...
    func show(settings: AppSettings? = nil) {
        // Resolve the default INSIDE the body: a default-argument expression is
        // evaluated in the caller's (possibly nonisolated) context, so touching
        // main-actor `AppEnvironment.shared` there trips isolation checking.
        let settings = settings ?? AppEnvironment.shared.settings
        if window == nil {
