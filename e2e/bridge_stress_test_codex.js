#!/usr/bin/env node
/*
 * Local WebSocket stress test for VinylPod's Swift BrowserBridge.
 *
 * Usage:
 *   node e2e/bridge_stress_test.js
 *   node e2e/bridge_stress_test.js --duration-ms 60000 --pid 12345
 *   node e2e/bridge_stress_test.js --process-name VinylPod --seek-hz 400 --binary-hz 800
 *
 * The script intentionally uses only Node.js built-ins. It implements the
 * client side of RFC 6455 directly so we can send masked JSON and binary frames
 * without depending on a WebSocket package.
 */
"use strict";
      ...
      socket.on("data", (chunk) => {
        if (!this.open) {
          ...
          if (!/^HTTP\/1\.1 101\b/.test(header) || !header.includes(`Sec-WebSocket-Accept: ${accept}`)) {
            fail(new Error(`${this.label}: bad handshake response: ${header.split("\r\n")[0] || "empty"}`));
            return;
          }
...
function nowPlayingPayload(sequence) {
  return {
    type: "nowplaying",
    payload: {
      source: sequence % 3 === 0 ? "spotify" : sequence % 3 === 1 ? "appleMusic" : "youtubeMusic",
...
async function runStableSeekStorm() {
  ...
  const timer = setInterval(() => {
    for (const client of stable) {
      client.sendText(nowPlayingPayload(sequence++));
    }
...
function printReport(final = false) {
  ...
  console.log([
    ...
    `backpressure=${metrics.clientBackpressureDrops}`,
    `serverFrames=${metrics.serverFramesReceived}`,
    `loopP99=${(loopDelay.percentile(99) / 1e6).toFixed(1)}ms`,
    rssText
  ].join(" "));
}
...
function summarizeAndExit() {
  ...
  const summary = {
    ...
    dropRate,
    receiverRssGrowthMb: rssGrowth
  };
  console.log(JSON.stringify(summary, null, 2));

  if (dropRate > cfg.failOnDropRate) process.exitCode = 2;
  if (rssGrowth > cfg.failOnRssGrowthMb) process.exitCode = 3;
}
...
function parseArgs(args, defaults) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (typeof out[key] === "number" || out[key] === null) {
      out[key] = Number(next);
      i += 1;
    } else {
      out[key] = next;
...
function printHelpAndExit() {
  console.log(`Usage: node e2e/bridge_stress_test.js [options]
...
Options:
  ...
  --binary-hz 500
  --binary-size 16384
  --churn-connections 250
  --churn-batch 10
  --stable-connections 4
  --pid <receiver pid>
  --process-name VinylPod
  --fail-on-drop-rate 0.05
  --fail-on-rss-growth-mb 150`);
  process.exit(0);
}
...
console.log(`Bridge stress test -> ws://${cfg.host}:${cfg.port}`);
...
setTimeout(() => {
  ...
  sampleReceiverRss();
  setTimeout(summarizeAndExit, 250);
}, cfg.durationMs);
